...
declare
    ...
    max_incident_count constant number := 4;
    max_incident_client_count constant number := 1;      
...
begin
    ...
    if (incident_count >= max_incident_count) then
    ...
    for client in clientlist loop
        select count(incidentid) into incident_client_count
            ...
            where extract(year from incident_date)=current_year
            and extract(month from incident_date)=current_month;
        if (incident_client_count >= max_incident_client_count) then
